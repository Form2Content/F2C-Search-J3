...
class Form2ContentSearchModelDatavw extends Form2ContentSearchModelSearchBase
{
	var $globalParams = null;
	var $itemCount = 0;
	var $dataViewId = null;
	var $searchFormId = null;
	var $dataView = null;
	
	public function __construct($config = array())
	{
		// get the Dataview we're displaying
		$db 	= JFactory::getDbo();
		...
		$query->where('id = ' . (int)$active->params->get('datavw_id'));

		$db->setQuery($query);
		
		$this->dataView = $db->loadObject();
		
		$this->dataViewId 	= $this->dataView->id;
		$this->searchFormId = $this->dataView->search_form_id;
		$this->globalParams = $this->getParams($this->searchFormId);
	...
	public function getListQuery()
	{
		...
		$formDataList = $f2cSearch->getFormIds(0, $this->searchFormId, $this->dataViewId);
