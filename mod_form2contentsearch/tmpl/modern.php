...
<form action="index.php" method="post" name="formModF2CSearch<?php echo $searchFormId; ?>" id="formModF2CSearch<?php echo $moduleId.'_'.$searchFormId; ?>">
	...
	<div id="f2cs_elements_table_<?php echo $moduleId; ?>"  class="f2cs_elements_table">
	<?php 
		for ($i = 0; $i < $form->numElements; $i++) :
			$element = $form->elements[$i];
			$float = ($form->numCols > 1 && ($i + 1) % $form->numCols <= $form->numCols);
	?>
		<div class="f2cs_element" <?php if ($float) echo 'style="float: left;"'; ?>>
	...
	<?php 
			if ($form->numCols > 1 && (int)(($i + 1) / $form->numCols)) :
	?>
